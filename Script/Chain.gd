...
onready var links = get_node("Links")		# A slightly easier reference to the links
onready var tipNode = get_node("Tip")
...
const SPEED = 75	# The speed with which the chain moves
...
func _process(_delta: float) -> void:
	...
	tipNode.rotation = self.position.angle_to_point(tip_loc) - deg2rad(90)
...
func _physics_process(_delta: float) -> void:
	tipNode.global_position = tip	# The player might have moved and thus updated the position of the tip -> reset it
	if flying:
		# `if move_and_collide()` always moves, but returns true if we did collide
		if tipNode.move_and_collide(direction * SPEED):
			hooked = true	# Got something!
			flying = false	# Not flying anymore
	tip = tipNode.global_position	# set `tip` as starting position for next frame
